...
class CheckerService(TaskManager):
    ...
    async def get_health_info(self, infohash, trackers=None, timeout=5) -> HealthInfo:
        # if not trackers:
        trackers = ['DHT']
    ...
    async def get_tracker_response(self, tracker, infohashes=None, timeout=20):
        ...
        #     # For DHT, only one torrent is handled at a time.
            tracker_response = await self.dht_tracker.get_torrent_health(infohashes[0], timeout=5)
        else:
