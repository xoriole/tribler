...
class UdpTracker(Tracker):
    ...
    async def get_tracker_response(self, tracker_url, infohashes, timeout=5, proxy=None) -> TrackerResponse:
        ...
        try:
            async with async_timeout.timeout(timeout):
                ...
                connection_id = await self.connect_to_tracker(ip_address, port, proxy)
                response_list = await self.scrape_response(ip_address, port, connection_id, infohashes, proxy)
    ...
    def compose_connect_request(self, host, port, proxy):
    ...
    async def connect_to_tracker(self, ip_address, port, proxy):
        connection_request = self.compose_connect_request(ip_address, port, proxy)
    ...
    def compose_scrape_request(self, host, port, connection_id, infohash_list, proxy):
        transaction_id = self.transaction_id
        self.transaction_id += 1
    ...
    async def scrape_response(self, ip_address, port, connection_id, infohash_list, proxy):
        scrape_request = self.compose_scrape_request(ip_address, port, connection_id, infohash_list, proxy)
