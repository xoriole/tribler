...
class DHTHealthManager(TaskManager):
    ...
    def dht_get_peers(self, infohash):
        ...
        print(f"Sending dht request for infohash: {infohash}")
    ...
    def finalize_lookup(self, infohash):
        ...
        """
        print(f"calling finalize on infohash: {hexlify(infohash)}")
        ...
        if infohash not in self.lookup_futures:
            print(f"no future found for the infohash")
            return
        ...
        print(f"dht manager health info: {health}")
