...
class Bootstrap(object):
    ...
    def __init__(self, config_dir, dht=None):
        ...
        self.bootstrap_dir = Path(config_dir, 'bootstrap')
        if not self.bootstrap_dir.exists():
            self.bootstrap_dir.mkdir(parents=True)
        self.dcfg.set_dest_dir(str(self.bootstrap_dir))
        self.bootstrap_file = self.bootstrap_dir.joinpath("bootstrap.blocks")
        self.nodes_file = self.bootstrap_dir.joinpath("bootstrap.nodes")
        self.dht = dht
