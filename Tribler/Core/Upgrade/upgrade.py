...
class TriblerUpgrader(object):
    ...
    def backup_state_directory(self):
        ...
        if self.session.config.get_version_backup_enabled() and self.session.config.get_version() \
            ...
            dest_state_dir = TriblerConfig.get_default_state_dir(version=self.session.config.get_version())
            ...
            if not os.path.exists(dest_conf_path):
                ...
                for backup_file in backup_files:
                    shutil.copy(os.path.join(src_state_dir, backup_file), os.path.join(dest_state_dir, backup_file))
